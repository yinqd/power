...
function loadData(){
  ...
  //如果没选,就加全部
    ...
    $.ajax({
        type: "POST",
        url: ctxPath + "dlscz/queryDlsczMonthReport.action",
        data: queryParameter,
        dataType: "json",
        ...
        success: function(data){
            ...
            if(items.length != 0){
            	for(var i=0;i<items.length;i++){
                    ...
                    html+='<td>'+obj.rechargeCount+'</td>';
                    html+='<td>'+obj.rechargePrice+'</td>';
                    html+='</tr>';
                }
            	 dlscztotal = "<tr><td align='center' colspan='2'>总计：</td><td colspan='1' align='center'>" + data.monthTotal.rechargeCount + "</td><td colspan='1' align='center'>" + data.monthTotal.rechargePrice + "</td></tr>";
...
function viewDetail(id){
    ...
    var url=path+"takeOutOrder/view?id="+id;
    window.open(url);
}
