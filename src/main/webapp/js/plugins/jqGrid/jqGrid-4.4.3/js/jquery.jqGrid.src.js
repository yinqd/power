...
(function ($) {
        ...
        return this.each(function () {
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                setPager = function (pgid, tp) {
                    ...
                    if (ts.p.pginput === true) {
                        pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/></td><td dir='" + dir + "'>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + "</td>";
                    }
                    if (ts.p.pgbuttons === true) {
    ...
    $.jgrid.extend({
        ...
        getCol: function (col, obj, mathopr, requirement) {
                    ...
                    if (ln && ln > 0) {
                        while (i < ln) {
                            if ($($t.rows[i]).hasClass('jqgrow')) {
                                ...
                                if (mathopr) {
                                    //计算表达式值决定当前值是否参与后续计算 sharpe
                                    if (requirement != '' && requirement != undefined) {
                                        var scope = {};
                                        scope[col] = val;
                                        if (!math.eval(requirement, scope)) {
                                            val = 0;
                                        }
                                    }
                                    //v = parseFloat(val);
                                    //sum += v;
                                    //使用math.js进行精确计算 sharpe
                                    v = math.bignumber(val == '' ? 0 : val);
                                    sum = math.add(math.bignumber(sum), v);
                                    if (max === undefined) {
                                        max = min = v;
                                    }
                                    min = Math.min(min, v);
        ...
        setSubGrid: function () {
            return this.each(function () {
                ...
                $t.p.colModel.unshift({
                    name: 'subgrid',
                    width: ($.browser.safari || $.browser.webkit) ? $t.p.subGridWidth + $t.p.cellLayout : $t.p.subGridWidth,
                    sortable: false,
                    resizable: false,
                    hidedlg: true,
                    search: false,
