...
//			return false;
	...
	changeDate: function(s,flag){
		var ms = 1000*60*60*24,
			days = ms*1*s,
			curd = strToDate(reportLayer.curDate.text())*1,//当前时间
...
//			console.log("newCurd == " + newCurd);
	...
	changeWeek: function(s,flag){
		var ms = 1000*60*60*24,
			...
			curStartD = strToDate(curd[0])*1,//开始时间
			curEndD = strToDate(curd[1])*1,//结束时间
...
//			console.log("newCurEndD == " + newCurEndD);
			//alert("strToDate(curd[0])"+(strToDate(curd[0]))+"strToDate(curd[0])*1====>"+(strToDate(curd[0])*1));
	...
	changeMonth: function(s,flag){
		var ms = 1000*60*60*24,
			curd = reportLayer.curDate.text().split("至"),//当前时间
			curYear = strToDate(curd[0]).getFullYear(),//开始年
			curMonth = strToDate(curd[0]).getMonth(),//开始月
			newCurYear,
			newCurMonth,
			days,
			newCurStartD,
